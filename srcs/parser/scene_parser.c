...
int	pars_camera(t_minirt *data, char **arr, int count)
{
	...
	if (cam->fov < 0 || cam->fov > 180)
		return (free_cam(data->cam), printerr_line(count, FOV_ERROR), 1);
	if (cam->fov == 0)
		return (free_cam(data->cam), printerr_line(count, MIN_FOV_ERROR), 1);
	if (cam->fov == 180)
		return (free_cam(data->cam), printerr_line(count, MAX_FOV_ERROR), 1);
	data->cam = cam;
